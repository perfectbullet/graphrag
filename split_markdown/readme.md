这里是一个按一级标题切分markdown文件的Python脚本：
...
1. 将脚本保存为 `split_markdown.py`

2. 在命令行中运行：
```bash
python split_markdown.py your_book.md
```

**脚本功能：**

- 按一级标题（`# 标题`）切分markdown文件
- 创建以输入文件名命名的文件夹
- 每个章节保存为单独的`.md`文件
- 文件名格式：`01_第一章.md`, `02_第二章.md` 等
- 自动清理标题中不适合文件名的字符
- 保留原始的markdown格式

**输出示例：**

如果输入文件是 `股票做手回忆录.md`，会创建 `股票做手回忆录` 文件夹，包含：
```
股票做手回忆录/
├── 01_第一章.md
├── 02_第二章.md
